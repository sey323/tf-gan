...
def make(folder_name, gray=False, img_size=0, train_num=0, test_num=0):
    ...
    for i, d in enumerate(classes):  # 1つのディレクトリに対する処理
        ...
        for j, file in enumerate(files):  # 1つのファイルに対する処理
            if (
                not "png" in file and not "jpg" in file and not "jpeg" in file
            ):  # jpg以外のファイルは無視
                continue
            # 画像読み込み
    ...
    if test_num != 0:
        train_batch, test_batch = _random_3sampling(
            train_images, train_labels, train_num=train_num, test_num=test_num
        )
