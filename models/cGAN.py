...
class cGAN( GAN ):
    ...
    def ConditionalDiscriminator(self,y,label,channel=3,reuse=False):
        ...
        with tf.variable_scope("Discriminator", reuse=reuse) as scope:
            ...
            k = tf.ones([self.batch_num,self.input_size[0],self.input_size[1],self.label_num])
            k = k * l
            ...
            for i , output_shape in enumerate( self.layers , 1 ):
                ...
                with tf.variable_scope("conv_layer{0}".format(i)) as scope:
                    h_conv     = layer.conv2d( x = before_output , stride=2 , filter_size = [self.filter_size[0],self.filter_size[1] , input_shape , output_shape], i = i ,batch_norm = False)
    ...
    def build_model(self):
        ...
        self.y_real = tf.placeholder( tf.float32,[None, self.input_size[0], self.input_size[1], self.channel],name="image")

        '''Generatorのネットワークの構築'''
        print('[BUILDING]\tGenerator')
        self.y_fake = self.ConditionalGenerator(self.z,self.label,self.channel)
        self.y_sample = self.ConditionalGenerator(self.z,self.label,self.channel,reuse=True)

        '''Discrimnatorのネットワークの構築'''
        print('[BUILDING]\tDiscriminator')
        self.d_real  = self.ConditionalDiscriminator(self.y_real,self.label,self.channel)
        self.d_fake  = self.ConditionalDiscriminator(self.y_fake,self.label,self.channel,reuse=True)
